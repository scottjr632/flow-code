...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect(
    "restores default permission mode on sendTurn when a non-full-access session exits plan mode",
    () => {
      const harness = makeHarness();
      return Effect.gen(function* () {
        const adapter = yield* ClaudeAdapter;
        ...
        harness.query.emit({
          ...
          is_error: false,
          errors: [],
          session_id: "sdk-session-default-restore",
          uuid: "result-default-restore",
        } as unknown as SDKMessage);
