...
interface WorkItemEditorValues {
        ...
        <DialogPanel className="space-y-3">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-1">
              ...
              >
                <SelectTrigger>{STATUS_META[values.status].label}</SelectTrigger>
                <SelectPopup>
...
export function WorkSurface({
  ...
  const openCreateDialog = () => {
    const nextValues = deriveEditorValues(
  ...
  const openEditDialog = (item: WorkItem) => {
    ...
    setEditorValues(deriveEditorValues({ mode: "edit", item }, item.projectId));
  };
