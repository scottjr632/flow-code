...
const BASE_DIR = process.env.FLOW_HOME?.trim() || Path.join(OS.homedir(), ".t3");
const STATE_DIR = Path.join(BASE_DIR, "userdata");
const DESKTOP_SCHEME = "t3";
const ROOT_DIR = Path.resolve(__dirname, "../../..");
const isDevelopment = Boolean(process.env.VITE_DEV_SERVER_URL);
const APP_DISPLAY_NAME = isDevelopment ? "Flow (Dev)" : "Flow (Alpha)";
const APP_USER_MODEL_ID = "com.t3tools.flow";
const USER_DATA_DIR_NAME = isDevelopment ? "flow-dev" : "flow";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "Flow (Dev)" : "Flow (Alpha)";
...
function backendChildEnv(): NodeJS.ProcessEnv {
  ...
  delete env.FLOW_PORT;
  delete env.FLOW_AUTH_TOKEN;
  delete env.FLOW_MODE;
  delete env.FLOW_NO_BROWSER;
  delete env.FLOW_HOST;
  delete env.FLOW_DESKTOP_WS_URL;
  return env;
}
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { flowCommitHash?: unknown };
    return normalizeCommitHash(parsed.flowCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.FLOW_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Flow failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const disabledReason = getAutoUpdateDisabledReason({
    ...
    disabledByEnv: process.env.FLOW_DISABLE_AUTO_UPDATE === "1",
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Flow ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/Flow (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`flow`). If the legacy
...
function configureAutoUpdater(): void {
  ...
  const githubToken =
    process.env.FLOW_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
  if (githubToken) {
  ...
  if (process.env.FLOW_DESKTOP_MOCK_UPDATES) {
    autoUpdater.setFeedURL({
      provider: "generic",
      url: `http://localhost:${process.env.FLOW_DESKTOP_MOCK_UPDATE_SERVER_PORT ?? 3000}`,
    });
  }
