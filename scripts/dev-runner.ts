...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("FLOW_PORT_OFFSET"),
  devInstance: optionalStringConfig("FLOW_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid FLOW_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `FLOW_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric FLOW_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed FLOW_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      FLOW_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.FLOW_PORT = String(serverPort);
      output.VITE_WS_URL = `ws://localhost:${serverPort}`;
    } else {
      delete output.FLOW_PORT;
      delete output.VITE_WS_URL;
      delete output.FLOW_AUTH_TOKEN;
      delete output.FLOW_MODE;
      delete output.FLOW_NO_BROWSER;
      delete output.FLOW_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.FLOW_HOST = host;
    }

    if (!isDesktopMode && authToken !== undefined) {
      output.FLOW_AUTH_TOKEN = authToken;
    } else if (!isDesktopMode) {
      delete output.FLOW_AUTH_TOKEN;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.FLOW_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.FLOW_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.FLOW_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.FLOW_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.FLOW_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.FLOW_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.FLOW_MODE = "web";
      delete output.FLOW_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read FLOW_PORT_OFFSET/FLOW_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("FLOW_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv("FLOW_AUTO_BOOTSTRAP_PROJECT_FROM_CWD"),
      logWebSocketEvents: readOptionalBooleanEnv("FLOW_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.FLOW_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.FLOW_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Flow data (equivalent to FLOW_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("FLOW_HOME")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to FLOW_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("FLOW_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to FLOW_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("FLOW_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to FLOW_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("FLOW_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to FLOW_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("FLOW_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to FLOW_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("FLOW_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to FLOW_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("FLOW_PORT")),
  ),
