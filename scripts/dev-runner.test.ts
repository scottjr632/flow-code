...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit FLOW_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "FLOW_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid FLOW_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults FLOW_HOME to ~/.t3 when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.FLOW_HOME, resolve(homedir(), ".t3"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.FLOW_HOME, resolve("/tmp/custom-t3"));
        assert.equal(env.FLOW_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.FLOW_NO_BROWSER, "1");
        assert.equal(env.FLOW_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.FLOW_LOG_WS_EVENTS, "1");
        assert.equal(env.FLOW_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            FLOW_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
        ...
        assert.equal(env.FLOW_MODE, "web");
        assert.equal(env.FLOW_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.FLOW_LOG_WS_EVENTS, "0");
      }),
    );
    ...
    it.effect("uses custom t3Home when provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.FLOW_HOME, resolve("/tmp/my-t3"));
      }),
    );
    ...
    it.effect("does not export backend bootstrap env for dev:desktop", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.FLOW_PORT, undefined);
        assert.equal(env.FLOW_AUTH_TOKEN, undefined);
        assert.equal(env.FLOW_MODE, undefined);
        assert.equal(env.FLOW_NO_BROWSER, undefined);
        assert.equal(env.FLOW_HOST, undefined);
